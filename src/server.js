...
// Middleware
const { createAuthMiddleware, createOptionalAuthMiddleware } = require('./api/middleware/auth');
...
async function startServer() {
  ...
  const m3uController = new M3uController(getUserM3U, cacheService);
  ...
  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: ["'self'", "http:", "ws:"] // Allow HTTP for mixed content
      }
    },
